...
if __name__ == '__main__':
    ...
    strain, stest, rtrain, rtest = \
                train_test_split(x_pca, Y, test_size = 0.3)
    ...
    '''cross_scores_svm = cross_val_score(model_svm, x_pca, Y, cv = 10)     #cv son los folders
    cross_scores_mlp = cross_val_score(model_mlp, x_pca, Y, cv = 10)     #cv son los folders

    print("cross_scores_svm:", cross_scores_svm.mean())
    print("cross_scores_mlp:", cross_scores_mlp.mean())'''

    model_svm.fit(strain, rtrain) 
    rpredict_svm = model_svm.predict(stest)

    model_mlp.fit(strain, rtrain) 
    rpredict_mlp = model_mlp.predict(stest)

    print(confusion_matrix(rtest, rpredict_svm))

    acc_score_svm = accuracy_score(rtest,rpredict_svm)*100.0
    acc_score_mlp = accuracy_score(rtest,rpredict_mlp)*100.0
    print('accuracy score: ', acc_score_svm)    
    print('accuracy score: ', acc_score_mlp)
